...
grating_dir = "/home/krg/Transfer_ESIS_Alignment_GSE/control_software/Output/" + grating_type + "/" + "grating_" + str(grating_num)
...
while(True):
    ...
    for nump in range(0,num_meas):
        ...
        for num in range(0,num_steps+1):
            ...
            for j in range(0, 1):        # Take intensity measurements
                raw = agilent.measure(voltmet, 101, 114)
                rawlist = raw.split(",")
                for k in range(0, len(rawlist)):
                    if k < 4:   # photodiode & Vin measurement
                        data.append(rawlist[k])
                    else:
                        data.append(steinhart(float(rawlist[k]), float(rawlist[3])))
                        if k == 13:
    ...
    eng.cd(r'../Matlab')
    eng.tmp(csv_fn, nargout=0)
    eng = matlab.engine.start_matlab()
    eng.cd(r'../Matlab')
    [cont_loop, start_pos, end_pos, num_steps, num_meas, iteration] = eng.TEA_GSE_Final_Align(csv_fn, iteration,grating_num,grating_type, nargout=6)
